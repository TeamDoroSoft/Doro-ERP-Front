...
export interface RequestOptions extends RequestInit {
  /** `If-Match` 원문 값(따옴표 포함). 기능마다 형식이 달라(예: `"product-3"`, `"42"`) 호출부가 직접 만든다. */
  ifMatch?: string
  /** 생성 API의 `Idempotency-Key` Header. */
...
export async function request<T>(path: string, options: RequestOptions = {}): Promise<T> {
  const { ifMatch, idempotencyKey, headers: providedHeaders, ...fetchOptions } = options
