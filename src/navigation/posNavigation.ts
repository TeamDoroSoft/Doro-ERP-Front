...
export interface PosNavigationItem {
  label: string
  to: string
  icon: 'orders' | 'tables' | 'queue' | 'fulfillment' | 'catalog' | 'sales' | 'audit' | 'settings'
  roles: EmployeeRole[]
  ready: boolean
  group: '운영' | '관리' | '분석'
}
...
export const posNavigation: PosNavigationItem[] = [
  ...
  { label: '입장 대기', to: '/pos/queues/entry', icon: 'queue', roles: allEmployees, ready: true, group: '운영' },
  {
    label: '조리 현황',
    to: '/pos/queues/fulfillment',
    icon: 'fulfillment',
    roles: allEmployees,
    ready: true,
    group: '운영',
