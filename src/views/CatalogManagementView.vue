...
const categoryDialog = ref<HTMLElement>()
const productDialog = ref<HTMLElement>()
...
watch(
  () => categoryEditor.value !== undefined || productEditor.value !== undefined,
  (open) => {
    if (typeof document === 'undefined') return
    if (open && previousBodyOverflow === undefined) {
      previousBodyOverflow = document.body.style.overflow
      document.body.style.overflow = 'hidden'
    } else if (!open && previousBodyOverflow !== undefined) {
...
async function openCategory(item: ManagedCategoryResponse | null, event?: Event) {
...
async function openProduct(item: ManagedProductResponse | null, event?: Event) {
  dialogTrigger = event?.currentTarget instanceof HTMLElement ? event.currentTarget : null
...
async function submitCategory() {
  await catalog.saveCategory(categoryEditor.value ?? undefined)
  if (!catalog.errorMessage.value) await closeDialog('category')
}
async function submitProduct() {
  if (await catalog.saveProduct(productEditor.value ?? undefined)) await closeDialog('product')
}
async function closeDialog(kind: 'category' | 'product') {
  if (catalog.busyId.value) return
  if (kind === 'category') categoryEditor.value = undefined
  else productEditor.value = undefined
  await nextTick()
  dialogTrigger?.focus()
  dialogTrigger = null
}
function handleDialogKeydown(event: KeyboardEvent, kind: 'category' | 'product') {
  if (event.key === 'Escape') {
    event.preventDefault()
    void closeDialog(kind)
    return
  }
  if (event.key !== 'Tab') return
  const dialog = kind === 'category' ? categoryDialog.value : productDialog.value
...
onBeforeUnmount(() => {
  dialogTrigger = null
  if (typeof document !== 'undefined' && previousBodyOverflow !== undefined) {
    document.body.style.overflow = previousBodyOverflow
    previousBodyOverflow = undefined
  }
})
...
<template>
  <main class="catalog-page">
    ...
    <ApiErrorNotice v-if="catalog.error.value && categoryEditor === undefined && productEditor === undefined" :message="catalog.errorMessage.value" :code="catalog.error.value.code" :request-id="catalog.error.value.requestId" retryable @retry="catalog.load" />
    <LoadingState v-if="catalog.loading.value" />
    <template v-else>
      <section class="catalog-card category-panel" aria-labelledby="category-list-title">
        <div class="section-heading"><div><div class="title-row"><h2 id="category-list-title">메뉴 분류</h2><span class="count" aria-label="메뉴 분류 수">{{ catalog.categories.value.length }}</span></div><p>분류를 선택하면 해당 상품을 확인할 수 있습니다.</p></div><button v-if="catalog.canManage.value" class="primary" type="button" @click="openCategory(null, $event)">분류 등록</button></div>
      ...
      <section class="catalog-card product-panel" aria-labelledby="product-list-title">
        ...
        <div v-else class="table-wrap"><table><thead><tr><th scope="col">상품</th><th scope="col">가격</th><th scope="col">판매 상태</th><th scope="col">재고 상태</th><th scope="col">작업</th></tr></thead><tbody><tr v-for="item in catalog.selectedProducts.value" :key="item.productId"><td><strong>{{ item.name }}</strong><small>{{ item.description }}</small></td><td class="price">{{ money(item.price) }}</td><td><span class="status" :class="{ inactive: !item.active }">{{ item.active ? '판매 중' : '판매 중지' }}</span></td><td><span class="status" :class="{ warning: item.soldOut }">{{ item.soldOut ? '품절' : '판매 가능' }}</span></td><td><div class="actions" aria-label="상품 작업"><button type="button" :disabled="!!catalog.busyId.value" @click="catalog.toggleSoldOut(item)">{{ item.soldOut ? '품절 해제' : '품절 처리' }}</button><button v-if="catalog.canManage.value" type="button" @click="openProduct(item, $event)">수정</button><button v-if="catalog.canManage.value" type="button" :disabled="!!catalog.busyId.value" @click="catalog.toggleProductActive(item)">{{ item.active ? '판매 중지' : '판매 재개' }}</button></div></td></tr></tbody></table></div>
      </section>
    </template>
    <Teleport to="body">
      <div v-if="categoryEditor !== undefined && catalog.canManage.value" class="modal-backdrop" @mousedown.self="closeDialog('category')">
        <section ref="categoryDialog" class="editor modal-dialog modal-dialog-compact" role="dialog" aria-modal="true" aria-labelledby="category-editor-title" @keydown="handleDialogKeydown($event, 'category')">
          <div class="editor-heading"><div><p class="eyebrow">{{ categoryEditor ? '선택한 분류 편집' : '새 분류 만들기' }}</p><h3 id="category-editor-title">{{ categoryEditor ? '메뉴 분류 수정' : '메뉴 분류 등록' }}</h3></div><button type="button" aria-label="메뉴 분류 편집 닫기" :disabled="!!catalog.busyId.value" @click="closeDialog('category')">닫기</button></div>
          ...
          <form @submit.prevent="submitCategory"><div class="featured-field"><label for="category-name">분류명</label><p id="category-name-help">고객과 직원이 메뉴를 찾을 때 표시되는 이름입니다.</p><input id="category-name" ref="categoryNameInput" v-model="catalog.categoryDraft.name" maxlength="100" aria-describedby="category-name-help" placeholder="예: 커피" /></div><label for="category-order">표시 순서<input id="category-order" v-model.number="catalog.categoryDraft.displayOrder" type="number" min="0" max="9999" /></label><label class="check"><input v-model="catalog.categoryDraft.active" type="checkbox" /> 운영 중</label><div class="form-actions"><button type="button" :disabled="!!catalog.busyId.value" @click="closeDialog('category')">취소</button><button class="primary" type="submit" :disabled="!!catalog.busyId.value">저장</button></div></form>
        </section>
      </div>
      <div v-if="productEditor !== undefined && catalog.canManage.value" class="modal-backdrop" @mousedown.self="closeDialog('product')">
        <section ref="productDialog" class="editor modal-dialog modal-dialog-wide" role="dialog" aria-modal="true" aria-labelledby="product-editor-title" @keydown="handleDialogKeydown($event, 'product')">
          <div class="editor-heading"><div><p class="eyebrow">{{ productEditor ? '선택한 상품 편집' : '새 상품 만들기' }}</p><h3 id="product-editor-title">{{ productEditor ? '상품 수정' : '상품 등록' }}</h3></div><button type="button" aria-label="상품 편집 닫기" :disabled="!!catalog.busyId.value" @click="closeDialog('product')">닫기</button></div>
          <ApiErrorNotice v-if="catalog.error.value" :message="catalog.errorMessage.value" :code="catalog.error.value.code" :request-id="catalog.error.value.requestId" retryable @retry="catalog.load" />
          <form @submit.prevent="submitProduct"><label>메뉴 분류<select ref="productCategoryInput" v-model="catalog.productDraft.categoryId"><option v-for="item in catalog.categories.value" :key="item.categoryId" :value="item.categoryId">{{ item.name }}</option></select></label><label>상품명<input v-model="catalog.productDraft.name" maxlength="100" /></label><label class="wide">설명<input v-model="catalog.productDraft.description" maxlength="500" /></label><label>가격<input v-model="catalog.productDraft.price" inputmode="numeric" pattern="(0|[1-9]\d*)" maxlength="9" /></label><label>표시 순서<input v-model.number="catalog.productDraft.displayOrder" type="number" min="0" max="9999" /></label><label class="check"><input v-model="catalog.productDraft.active" type="checkbox" /> 판매 중</label><div class="form-actions"><button type="button" :disabled="!!catalog.busyId.value" @click="closeDialog('product')">취소</button><button class="primary" type="submit" :disabled="!!catalog.busyId.value">저장</button></div></form>
        </section>
      </div>
    </Teleport>
  </main>
...
.modal-backdrop{position:fixed;z-index:1000;inset:0;display:grid;place-items:center;overflow-y:auto;background:rgb(15 23 42 / 58%);padding:24px}.modal-dialog{display:grid;width:min(100%,680px);max-height:calc(100dvh - 48px);box-sizing:border-box;gap:18px;overflow-y:auto;border:1px solid var(--color-border-strong);border-radius:var(--radius-surface);background:var(--color-surface);padding:20px;box-shadow:0 24px 70px rgb(15 23 42 / 28%)}.modal-dialog-compact{width:min(100%,460px)}.editor-heading{justify-content:space-between;gap:12px}.editor-heading h3{margin-top:3px;color:var(--color-heading);font-size:16px}.modal-dialog form{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));align-items:end;gap:12px}.modal-dialog-compact form{grid-template-columns:1fr}.modal-dialog label,.featured-field{display:grid;gap:6px;font-weight:700}.modal-dialog input,.modal-dialog select{width:100%;min-height:40px;box-sizing:border-box;border:1px solid var(--color-border-strong);border-radius:var(--radius-control);background:white;padding:0 10px;font:inherit}.featured-field{grid-column:1/-1;border:1px solid var(--color-border-strong);border-radius:var(--radius-control);background:var(--color-surface-subtle);padding:14px}.modal-dialog-compact .featured-field{grid-column:auto}.featured-field p{margin:0;color:var(--color-muted);font-size:12px;font-weight:400}.featured-field input{border-width:2px}.check{display:flex!important;align-items:center;align-self:center}.check input{width:auto;min-height:auto}.form-actions{grid-column:1/-1;justify-content:flex-end;gap:8px;border-top:1px solid var(--color-border);padding-top:12px}.wide{grid-column:1/-1}
...
@media(max-width:900px){.catalog-page{grid-template-columns:220px minmax(0,1fr)}}@media(max-width:700px){.catalog-page{grid-template-columns:1fr}.catalog-page>header,.catalog-page>.notice,.catalog-page>.api-error,.catalog-page>.loading-state{grid-column:auto}.page-heading,.section-heading{align-items:stretch;flex-direction:column}.page-heading>button,.section-heading>button{align-self:flex-start}.category-panel{border-right:0;border-bottom:1px solid var(--color-border)}.category-list li{grid-template-columns:minmax(0,1fr) auto;align-items:center}.category-list .actions{border-top:0;padding-top:0}.empty{padding:32px 12px}.modal-backdrop{place-items:start center;padding:12px}.modal-dialog{max-height:calc(100dvh - 24px)}.modal-dialog-wide form{grid-template-columns:1fr}.modal-dialog-wide .featured-field,.modal-dialog-wide .wide{grid-column:auto}}@media(max-width:460px){.catalog-card,.page-heading{padding:14px}.category-list li{grid-template-columns:1fr}.category-list .actions{border-top:1px solid var(--color-border);padding-top:8px}.modal-dialog{padding:16px}.form-actions button{flex:1}}
</style>
